...
int GetqqliveSopath(char * szsearchdir,char * folder)
{
	DIR *dir = 0;
	struct dirent *ptr = 0;
	...
	while ((ptr=readdir(dir)) != NULL)
	{
		...
		else if(ptr->d_type == 4)
		{
			if( strcmp(ptr->d_name,"..") == 0 || strcmp(ptr->d_name,".") == 0){
				continue;
			}else if(strstr(ptr->d_name,"p2p_P2P.")){
...
int qqliveProcess(char * workpath,const char * szCmdline){
	int ret = 0;
	char tmppath[PATH_MAX] = {0};
    strcpy(tmppath,"/data/data/");
    strcat(tmppath,szCmdline);
    strcat(tmppath,"/cache_so/");
    ret = GetqqliveSopath(tmppath,workpath);
    if(ret < 0){
    	return ret;
    }
